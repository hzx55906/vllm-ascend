...
void AddRmsNormDynamicQuantAGTilingHelper::SetTilingDataAndTilingKeyAndWorkSpace(AddRmsNormDynamicQuantAGTilingData* tiling)
{
    ...
    // auto rankIdPtr = attrs->GetAttrPointer<int>(ATTR_RANK_ID_INDEX);
    ...
    uint64_t userWorkSpace = tempHeadCoreNum * rowLen * (*groupSizePtr) * sizeof(uint8_t) + tempHeadCoreNum * (*groupSizePtr)* sizeof(uint32_t);
    ...
    // std::string algConfig = "AlltoAll=level0:fullmesh";
    ...
    if (socVersion == platform_ascendc::SocVersion::ASCEND910B) {
        std::string algConfig = "AlltoAll=level0:fullmesh";
    } else if(socVersion == platform_ascendc::SocVersion::ASCEND910_93) {
    ...
    currentWorkspace[0] = this->sysWorkspaceSize_ + usrSize + userWorkSpace;
    ...
    OP_LOGI(
        "Set TilingDataAndTilingKeyAndWorkSpace", "Tilingdata tilingKey = %u, usr Workspace: %zu, group_size:%u, userWorkSpace: %zu", tilingKey, usrSize, *groupSizePtr,userWorkSpace);
}

bool AddRmsNormDynamicQuantAGTilingHelper::DoTiling()
{
    OP_TILING_CHECK(
        (nullptr == context_), OP_LOGE("AddRmsNormDynamicQuantAGTiling", "Helper context_ get nullptr, return failed."),
        return false);
...
bool AddRmsNormDynamicQuantAGTilingHelper::ValidateBaseParameters()
{
    OP_TILING_CHECK(
        this->eps_ <= 0,
        OP_LOGE(context_->GetNodeName(), "Epsilon less or equal than precision threshold, please check."),
        return false);
    OP_TILING_CHECK(
...
bool AddRmsNormDynamicQuantAGTilingHelper::ValidateInputOutput()
{
    ...
    if ((ge::GRAPH_SUCCESS != CheckDtypeVaild(y1DataType, supportedYDtypes)) ||
        ...
        OP_LOGE(context_->GetNodeName(), "Output dtype should be int8 int4 hifp8 and y1DataType y2DataType need same.");
        return false;
    }
...
ge::graphStatus Tiling4AddRmsNormDynamicQuantAG(gert::TilingContext* context)
{
    ...
    OP_LOGI(context->GetNodeName(), "Enter Tiling4AddRmsNormDynamicQuantAG");
...
{
    OP_TILING_CHECK(nullptr == context, OP_LOGE("AddRmsNormDynamicQuantAG", "Context is null"), return ge::GRAPH_FAILED);
    OP_LOGD(context->GetNodeName(), "Enter TilingPrepare4AddRmsNormDynamicQuantAG.");
