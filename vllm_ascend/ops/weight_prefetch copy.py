...
SUPPORTED_MODULES = ["attn", "mlp", "moe", "lm_head"]
...
class WeightPrefetchMethod:
    ...
    def __init__(self, weight_prefetch_config: WeightPrefetchConfig) -> None:
        ...
        self.lm_head = ModuleWeightPrefetchConfig(
            module_name="lm_head",
            enable=weight_prefetch_config.enabled,
            prefetch_ratio=weight_prefetch_config.prefetch_ratio.get("lm_head", {}) or {"weight": 1.0},
        )
    ...
    def maybe_prefetch_moe_weight_preprocess(self, hidden_states, prefix):
        ...
        # Use lm_head prefetch ratio when prefetching lm_head.weight at the last layer
        if _EXTRA_CTX.prefetch_lm_head:
            prefetch_ratio = self.lm_head.prefetch_ratio.get("weight", 0)
        else:
            prefetch_ratio = self.moe.prefetch_ratio.get(prefix, 0)
        weight_size = weight.data.element_size() * weight.data.numel() * prefetch_ratio
    ...
    def maybe_prefetch_moe_weight_postprocess(self, stop_flag: torch.Tensor):
        ...
        # Skip postprocess for lm_head prefetch — the wait will happen
        # later in AscendLogitsProcessor._get_logits to maximize overlap.
        if _EXTRA_CTX.prefetch_lm_head:
            return
    ...
    def _get_moe_gate_up_prefetch_weight(self) -> torch.Tensor | None:
        ...
        if layer_idx is None or model_instance is None:
            return None

        num_layers = len(model_instance.model.layers)

        # Last layer: prefetch lm_head.weight instead of next layer's w13_weight
        if layer_idx >= num_layers - 1:
            if self.lm_head.enable:
                lm_head = getattr(model_instance, "lm_head", None)
                if lm_head is not None:
                    _EXTRA_CTX.prefetch_lm_head = True
    ...
    def _maybe_prefetch_mlp_gate_up_weight_preprocess(
        ...
        if curr_layer_prefix.split(".")[-2] == "self_attn":
            ...
            # Last layer: prefetch lm_head.weight instead of next layer's gate_up_proj
            if layer_idx >= num_layers - 1 and self.lm_head.enable:
                lm_head = getattr(model_instance, "lm_head", None)
                if lm_head is not None:
                    weight = lm_head.weight
                    weight_size = (
                        weight.data.element_size() * weight.data.numel()
                        * self.lm_head.prefetch_ratio.get("weight", 0)
                    )
                    torch.ops.vllm.prefetch_preprocess(
                        weight=weight, start_flag=x_dependency, max_weight_size=int(weight_size)
                    )
                    _EXTRA_CTX.prefetch_lm_head = True
                    return
    ...
    def maybe_prefetch_mlp_weight_postprocess(self, stop_flag: torch.Tensor):
        ...
        # Skip postprocess for lm_head prefetch — wait happens in AscendLogitsProcessor
        if _EXTRA_CTX.prefetch_lm_head:
            return
    ...
    def maybe_prefetch_lm_head_weight_postprocess(self, stop_flag: torch.Tensor):
        """Wait for lm_head weight prefetch to complete.

        Called just before lm_head.quant_method.apply() in
        AscendLogitsProcessor._get_logits to ensure the weight
        is ready for the GEMM.
        """
        if not _EXTRA_CTX.prefetch_lm_head:
            return
        torch.ops.vllm.prefetch_postprocess(stop_flag)
        _EXTRA_CTX.prefetch_lm_head = False
