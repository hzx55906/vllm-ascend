...
{
    OP_LOGI(context, "Begin to do InferShape4AddRmsNormDynamicQuantAG");
    ...
    uint32_t rankNum = 0;
    // if (*groupSizePtr <= 0) {
    //     if ((Mc2Hcom::MC2HcomTopology::CommGetInstSizeByGroup(group, &rankNum)) != HCCL_SUCCESS || rankNum == 0) {
    //         OP_LOGE(
    //             context->GetNodeName(), "Get rank size failed, group [%s], rankSize [%u]", group, rankNum);
    //         return GRAPH_FAILED;
    //     } 
    // } else {
    ...
    OP_LOGI(context, "InferShape4AddRmsNormDynamicQuantAG  groupSizePtr:%d ranknum: %d", *groupSizePtr, rankNum);
    *y1Shape = *x1Shape;
    ...
    *xShape = *x1Shape;
    ...
    OP_CHECK_IF(isOnlyExistSmooth2, OP_LOGE(context, "Dynamic AddRmsNormDynamicQuantAG Not support only have scale2."),
                return GRAPH_FAILED);
    // unknown rank
    if (IsUnknownRank(x1Shape) || IsUnknownRank(gammaShape)) {
        ...
        OP_LOGI(context, "End to do InferShape4AddRmsNormDynamicQuantAG with unknown rank.");
        return GRAPH_SUCCESS;
    }

    auto ret = InferReduceShape(x1Shape, gammaShape, outScale1Shape);
    outScale1Shape->SetDim(0, outScale1Shape->GetDim(0) * rankNum);
    OP_CHECK_IF(!ret, OP_LOGE(context, "Dynamic AddRmsNormDynamicQuantAG Not support gammaDimNum > xDimNum."),
                return GRAPH_FAILED);
    if (smooth2Exist) {
        *outScale2Shape = *outScale1Shape;
        *y2Shape = *y1Shape;
    ...
    OP_LOGI(context, "InferShape4AddRmsNormDynamicQuantAG y1 shape:%d xx %d,s1 shape:%d", y1Shape->GetDim(0), y1Shape->GetDim(1),outScale1Shape->GetDim(0));

    OP_LOGI(context, "End to do InferShape4AddRmsNormDynamicQuantAG");
    return GRAPH_SUCCESS;
}
...
static graphStatus InferDataType4AddRmsNormDynamicQuantAG(gert::InferDataTypeContext* context)
{
    ...
    // yDtype = context->GetOutputDesc(Y1_IDX)->GetDataType();
    // OP_CHECK_IF(std::find<std::initializer_list<ge::DataType>::const_iterator, ge::DataType>(OUT_TYPE_LIST.begin(), OUT_TYPE_LIST.end(), yDtype) == OUT_TYPE_LIST.end(),
    //     (OP_LOGE(context,"attr dst_type only support 2(int8), 3(int4), 34(hifloat8), 35(float8_e5m2), 36(float8_e4m3fn)"),
